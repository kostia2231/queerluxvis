...
export default function Header() {
  ...
  return (
    <>
      ...
      >
        <div
          className="transition-colors cursor-pointer py-5 border-b bg-white border-x hover:bg-gray-100"
          onClick={() => router.push("/")}
        >
          <p className="">QLV</p>
        ...
        <div className="max-[450px]:hidden">
          <div className="transition-colors border-x cursor-pointer bg-white hover:bg-[#FF59A8] hover:text-white border-b">
            <Link href={"/donation"} className="w-full h-full block py-5">
              Support us!
            </Link>
          </div>
        </div>
        ...
        <div className="justify-between items-center">
          <div
            onClick={() => openCart()}
            className="transition-colors bg-gray-100 py-5 flex gap-2.5 items-center cursor-pointer hover:bg-[#FF59A8] hover:text-white border-b"
          >
            <p>Cart</p>
            {count > 0 && <p>— {count}</p>}
